        ...
        <div className="absolute bottom-2 right-2 z-20">
          {inCart ? (
            <div className="flex items-center gap-0.5 rounded-full bg-white p-1 shadow-soft ring-1 ring-ink-100">
              <button
                onClick={onDec}
                className="touch-target grid h-8 w-8 place-items-center rounded-full text-ink-700 transition-colors hover:bg-ink-100"
              ...
              <button
                onClick={onInc}
                className="touch-target grid h-8 w-8 place-items-center rounded-full bg-brand-500 text-ink-900 transition-colors hover:bg-brand-400"
            ...
            <button
              onClick={onAdd}
              className="touch-target grid h-9 w-9 place-items-center rounded-full bg-white p-1 shadow-soft ring-1 ring-ink-100 transition-colors"
    ...
    <div className="flex min-h-[96px] items-stretch gap-3 overflow-hidden rounded-2xl border border-ink-100 bg-white p-1.5 sm:p-0">
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between py-2 pr-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <button
            onClick={onRemove}
            className="touch-target grid h-9 w-9 shrink-0 place-items-center rounded-md text-ink-400 transition-colors hover:bg-red-50 hover:text-red-500"
        ...
        <div className="mt-1 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={onDec}
              className="touch-target grid h-9 w-9 place-items-center rounded-full border border-ink-200 bg-white text-ink-700 transition-colors hover:bg-ink-50"
              aria-label="Decrease"
            >
              <Minus className="h-3.5 w-3.5" />
            </button>
            ...
            <button
              onClick={onInc}
              className="touch-target grid h-9 w-9 place-items-center rounded-full bg-ink-900 text-white transition-colors hover:bg-ink-800"
              aria-label="Increase"
            >
              <Plus className="h-3.5 w-3.5" />
            </button>
...
function CheckoutSummary({
  ...
  return (
    <div className="space-y-3 border-t border-ink-100 px-5 py-4">
              ...
              <div className="flex items-center gap-1.5 px-1">
                <div className="inline-flex shrink-0 overflow-hidden rounded-xl border border-ink-200 bg-white">
                  <button
                    ...
                    className={`touch-target grid h-9 w-9 place-items-center transition-colors focus:outline-none ${
                ...
                />
                <button onClick={handleApply} className="btn-primary h-10 shrink-0 rounded-xl px-4 text-xs font-bold">
                  Apply
                </button>
              </div>
            </div>
      ...
      >
        Proceed to Payment
        <kbd className="ml-2 hidden rounded border border-ink-200/40 bg-white/20 px-1.5 py-0.5 font-mono text-[10px] font-semibold sm:inline">
          ⌘ ↵
        </kbd>
      </button>
    </div>
...
export default function POSPage() {
  ...
  // ---- Keyboard shortcuts ----------------------------------------------
  // Cashier speed: Cmd/Ctrl+Enter jumps straight to payment from anywhere
  // outside an input. ESC clears the cart when it's not empty.
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
  ...
  return (
    <div className="min-h-screen bg-ink-50 px-3 py-3 sm:px-5 sm:py-5">
      <div className="mx-auto flex h-[calc(100dvh-1.5rem)] max-w-[1400px] flex-col gap-4">
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_360px] xl:grid-cols-[minmax(0,1fr)_400px]">
          ...
          <section className="flex min-h-0 min-w-0 flex-col overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-soft">
            ...
            <div className="pos-scroll min-h-0 flex-1 overflow-y-auto p-3 sm:p-5">
      ...
      <button
        ...
        className="fixed bottom-4 right-4 z-40 inline-flex min-h-[52px] items-center gap-2 rounded-pill bg-ink-900 px-5 py-3 text-sm font-semibold text-white shadow-pop hover:bg-ink-800 lg:hidden"
      ...
      >
        ...
        Cart
        {cartCount > 0 && (
          <span className="grid h-6 min-w-6 place-items-center rounded-full bg-brand-500 px-2 text-[11px] font-bold text-ink-900">
            {cartCount}
          </span>
        )}
      </button>
